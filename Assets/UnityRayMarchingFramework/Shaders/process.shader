...
#include "ShaderLibs/SDF.cginc"
			...
			float2 Map(float3 pos){
				float dist1 = SdSphere(pos, 1.2);// OpS(SdSphere(pos, 1.2), SdBox(pos, float3(1, 1, 1)));
				float dist2 = OpS(SdBox(pos, float3(1, 1, 1)), SdSphere(pos, 1.2));

				return float2(lerp(dist1, dist2, _process), SPHERE_ID);
			}
			...
			float3 ShadingShpere(float3 rd,float3 pos, float3 n,float3 sd){
				...
				float bklig = clamp(dot(n,-lightDir),0.,1.)*0.05+0.3;//加点背光
